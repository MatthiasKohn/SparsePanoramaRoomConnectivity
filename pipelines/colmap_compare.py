...
Workflow:
  ...
  3. converts COLMAP relative poses to SE(2) edges (metric via the fitted scale) for
     sparsepano/pose/posegraph -> ready to fuse with door-embedding edges (hybrid pose graph).

  python -m pipelines.colmap_compare --home ../data/zind/full_dataset/0330 \
      --model <colmap_sparse_dir>
  python -m pipelines.colmap_compare --home ../data/zind/full_dataset/0330 --selftest
"""
...
def main(a):
    ...
    print(f"  -> {len(edges_se2)} COLMAP relative-pose edges ready for sparsepano/pose/posegraph "
