...
- `ContextMDs/ProjectOverview.md` — problem, hypothesis, current pipeline/track, status, headline results.
- `ContextMDs/Roadmap.md` — Paper 1 + Paper 2 plans, eval protocol, open questions, next actions.
- `ContextMDs/ResearchLog.md` — distilled dated findings (the evidence trail).
- `ContextMDs/RelatedWork.md` — positioning + the 2026 foundation-model landscape.
- `TODO.md` — open items. `CLUSTER.md` + `scripts/env_leonardo.sh` — cluster ops.
- `overlap_probe/` — the clean foundation-model pose benchmark (also the style template for new code).
- `CODEX_REFACTOR_PROMPT.md` — the pending repo-restructure spec.
...
**CURRENT FOCUS (edit me):** repo restructure (CODEX_REFACTOR_PROMPT.md); then adopt PaGeR + cubemap-face
door detection to lift connectivity recall. Ask before starting multi-step work.

---

## Keeping docs current
Single source of truth = the 4 `ContextMDs/` docs. When a result lands or a decision is made, append to
