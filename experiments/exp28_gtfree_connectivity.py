...
src/door_semantic.SemanticDoorDetector (SegFormer ring views, RGB only, no depth,
...
Detections are cached per home (json) so re-runs with different scoring are free.
  ...
  # headline (GPU):
  python experiments/exp28_gtfree_connectivity.py --root <ZIND_ROOT> \
  ...
  python experiments/exp28_gtfree_connectivity.py ... --doors gt --tag hardneg_gt

  # CPU plumbing self-test (fake segmenter, fake embedder):
  python experiments/exp28_gtfree_connectivity.py --home <HOME_DIR> --selftest
"""
...
import config
from src import zind, panoproj, door_dataset
