...
Examples:
  ...
  python experiments/exp14_export_pointcloud.py --dataset zind --all --merge --tint --preview
  # auto-align ALL Immersight panos (no GT) and look at the result:
  python experiments/exp14_export_pointcloud.py --dataset immersight --all --estimate --tint --preview --max 8
  # one Immersight room:
  python experiments/exp14_export_pointcloud.py --dataset immersight --stems 1273546
...
def load(dataset, stem, prov):
    ...
    if dataset == "stanford":
        from src import stanford as st
